...
get.notes <- function(dt, file, verbose = F) {
  ...
  # Returns:
  #   dt: list of data.tables
  dt$notes <- read.data(file, folder = dt$folderInput)
  ...
  sapply(unique(dt$notes[, Currency]), function (ccy) {
    adjustmentCcy <- dt$notes[Note == adjustmentColName & Currency == ccy]
  ...
  dt$adjustmentHUF <- sum(dt$notes[Note == adjustmentColName, AmountHUF])
  dt$adjustmentUSD <- sum(dt$notes[Note == adjustmentColName, AmountUSD])
  dt$notesHUF <- sum(dt$notes[Note != adjustmentColName, AmountHUF])
  dt$notesUSD <- sum(dt$notes[Note != adjustmentColName, AmountUSD])
  if (verbose) {
